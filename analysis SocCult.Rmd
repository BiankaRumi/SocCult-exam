...
combined<-read.csv("combined_test.csv")
...
combined$d<-ifelse(is.na(combined$d_data), F, T)

combined$f<-ifelse(is.na(combined$f_data), F, T)

combined$q<-ifelse(is.na(combined$q_data), F, T)
class(combined$q)
...
#First, I will define my models:
 # I cannot design the priors  and the family individually in this so I try to make different formulas for each

f0_d<-bf(d_data ~ 1 + d_condition) #bernoulli
f0_q<-bf(q_data ~ 1 + q_condition)#gaussian
f0_f<-bf(f_data ~ 1 + f_condition) #gaussian
...
f0_q_prior<-c(
  ...
  prior(normal(3.5, 1), class = sigma)
  )
...
get_prior(f1_q, combined, gaussian())
...
get_prior(f2_q, combined, gaussian())
...
get_prior(f0_f, combined, gaussian())
mean(combined$f_data,na.rm=TRUE)
sd(combined$f_data,na.rm=TRUE)
f0_f_prior<-c(
  prior(normal(3.7, 1.2), class = Intercept),
  prior(normal(1.2, .3), class = b), 
...
get_prior(f1_f, combined, gaussian())
mean(combined$f_data,na.rm=TRUE)
sd(combined$f_data,na.rm=TRUE)
f1_f_prior<-c(
...
get_prior(f2_f, combined, gaussian())
mean(combined$f_data,na.rm=TRUE)
sd(combined$f_data,na.rm=TRUE)
...
f3<- bf(mvbind(d_data,q_data,face_data) ~ 1 + CPI_native + CPI_change + condition CPI_change:Years + (1 + condition | ID))
